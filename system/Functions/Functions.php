...
if (!function_exists('database')) {
    ...
    function database($connection, $key = null)
    {

        $app = include dirname(__DIR__, 2) . '/config/database.php';

        return ($app['connections'][$connection][$key]) ?? $app['connections'][$connection];
    }
...
if (!function_exists('base_url')) {
    ...
    function base_url($path = null)
    {
        ...
        $url = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? 'https://' : 'https://') . $_SERVER['HTTP_HOST'] . str_replace('/index.php', '', $_SERVER['SCRIPT_NAME']);
