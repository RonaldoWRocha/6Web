$(function() {
  ...
  $('.ajaxForm').submit(function(event) {
    ...
    $.ajax({
      ...
      success: function(response) {
        ...
        */
        returnErrosValidation(response);
  ...
  // FIM DO AjaxDELETE
  function returnErrosValidation(response){
    ...
    if (response.return) {
      ...
      const erros = response.return;
      const keys = Object.keys(erros);

      for(let key of keys){
        $(`.validation.${key}`).html(erros[key]);
      }
  ...
  $(document).on("click", ".ajaxBtnRequest", function () {
